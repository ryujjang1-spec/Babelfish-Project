...
async function createRealtimeSession(_req: express.Request, res: express.Response) {
  ...
  if (!apiKey) {
    res.status(500).json({
      ...
      message: "서버 연결 실패 / API Key 확인 필요: OPENAI_API_KEY가 서버 .env에 설정되어 있지 않습니다."
    });
    return;
  }
          ...
          audio: {
            ...
            input: {
              ...
              transcription: {
                ...
                language: "ko",
                prompt:
                  "한국어 온디멘드 컨시어지 대화입니다. 택시, 병원 예약, 자동차 정비, 자동차 검사, 블랙박스, 틴팅, 상품 구매 관련 고객 말씀을 정확히 받아 적어 주세요. 소음이나 Babelfish 음성 에코는 고객 발언으로 확정하지 마세요."
              },
              turn_detection: {
                type: "semantic_vad",
                eagerness: "medium",
...
function createMockOrder(req: express.Request, res: express.Response) {
  ...
  if (analysis.escalationRequired) {
    res.status(409).json({ status: "operator_transfer", analysis, message: "운영자 확인이 필요한 요청입니다." });
    return;
  }
